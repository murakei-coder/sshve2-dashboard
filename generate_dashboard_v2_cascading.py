"""
Generate SSHVE2 Dashboard V2 with Cascading Filters and Zero OOS/VRP Coefficients
- OOS and VRP Missing coefficients set to 0
- Cascading filter hierarchy: Team > Mgr > Alias > MCID = Merchant Name
- Merchant Name filter added
- Optimized: Pre-aggregated suppression data (reduced file size from 145MB to ~3MB)
"""
import json
import pandas as pd
...
output = f'sshve2_dashboard_v2_cascading_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html'
...
<p style="margin:5px 0 0 0;font-size:0.75em;color:#FF6B6B;font-style:italic">※SSHVE1データが全て0%のセラーには全CEセラー平均値を使用</p>
</div>
<div>
...
<div class="bulk-group"><label>OOS改善率%</label><input type="number" id="bulkOOS" placeholder="0" step="0.1"></div>
...
case 8:valA=a.current_gms+(a.event_participation.past_participation?.sshve1_gms||0)+(a.event_participation.past_participation?.t365_gms||0);valB=b.current_gms+(b.event_participation.past_participation?.sshve1_gms||0)+(b.event_participation.past_participation?.t365_gms||0);break;
case 9:valA=a.event_participation.past_participation?.sshve1_gms||0;valB=b.event_participation.past_participation?.sshve1_gms||0;break;
case 10:valA=a.event_participation.past_participation?.t365_gms||0;valB=b.event_participation.past_participation?.t365_gms||0;break;
case 11:valA=a.sshve1_suppression['No suppression'];valB=b.sshve1_suppression['No suppression'];break;
case 12:valA=a.sshve1_suppression['OOS'];valB=b.sshve1_suppression['OOS'];break;
case 13:valA=a.sshve1_suppression['VRP Missing'];valB=b.sshve1_suppression['VRP Missing'];break;
case 14:valA=a.sshve1_suppression['Price Error'];valB=b.sshve1_suppression['Price Error'];break;
case 15:valA=a.sshve1_suppression['Others'];valB=b.sshve1_suppression['Others'];break;
case 16:valA=a.sshve2_suppression['No suppression'];valB=b.sshve2_suppression['No suppression'];break;
case 17:valA=a.sshve2_suppression['OOS'];valB=b.sshve2_suppression['OOS'];break;
case 18:valA=a.sshve2_suppression['VRP Missing'];valB=b.sshve2_suppression['VRP Missing'];break;
case 19:valA=a.sshve2_suppression['Price Error'];valB=b.sshve2_suppression['Price Error'];break;
case 20:valA=a.sshve2_suppression['Others'];valB=b.sshve2_suppression['Others'];break;
case 21:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.forecastOPS;break;}}
case 22:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.forecastOPS-cA.targetOPS;break;}}
case 23:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.targetOPS;break;}}
case 24:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.srcImp;break;}}
case 25:valA=userInputs[a.cid]?.oosImprovement||0;valB=userInputs[b.cid]?.oosImprovement||0;break;
case 26:valA=userInputs[a.cid]?.priceErrorReduction||0;valB=userInputs[b.cid]?.priceErrorReduction||0;break;
case 27:valA=userInputs[a.cid]?.otherSuppReduction||0;valB=userInputs[b.cid]?.otherSuppReduction||0;break;
case 28:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.suppImp;break;}}
case 29:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.projOPS;break;}}
case 30:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.incOPS;break;}}
case 31:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.vsTarget;break;}}
default:return 0;
}}
if(colIndex>=21&&colIndex<=31){{
const cB=calcProjectedOPS(b.cid,b.current_gms,b.target_gms,b.sshve1_suppression,b.sshve2_suppression);
switch(colIndex){{
case 21:valB=cB.forecastOPS;break;
case 22:valB=cB.forecastOPS-cB.targetOPS;break;
case 23:valB=cB.targetOPS;break;
case 24:valB=cB.srcImp;break;
case 28:valB=cB.suppImp;break;
case 29:valB=cB.projOPS;break;
case 30:valB=cB.incOPS;break;
case 31:valB=cB.vsTarget;break;
}}
}}
...
h+='<th rowspan="2" onclick="sortTable(24)">Sourcing<br>改善幅'+getSortIndicator(24)+'</th>';
h+='<th rowspan="2" onclick="sortTable(25)">OOS<br>改善率%'+getSortIndicator(25)+'</th>';
h+='<th rowspan="2" onclick="sortTable(26)">Price Error<br>削減率%'+getSortIndicator(26)+'</th>';
h+='<th rowspan="2" onclick="sortTable(27)">その他Supp<br>削減率%'+getSortIndicator(27)+'</th>';
h+='<th rowspan="2" onclick="sortTable(28)">Suppression<br>改善幅'+getSortIndicator(28)+'</th>';
h+='<th rowspan="2" onclick="sortTable(29)">Projected<br>OPS'+getSortIndicator(29)+'</th>';
h+='<th rowspan="2" onclick="sortTable(30)">Incremental<br>OPS'+getSortIndicator(30)+'</th>';
h+='<th rowspan="2" onclick="sortTable(31)">vs Target<br>(Projected OPS-Target OPS)'+getSortIndicator(31)+'</th></tr>';
h+='<tr>';
...
h+='<th onclick="sortTable(8)">Total GMS'+getSortIndicator(8)+'</th>';
h+='<th onclick="sortTable(9)">SSHVE1'+getSortIndicator(9)+'</th>';
h+='<th onclick="sortTable(10)">T365'+getSortIndicator(10)+'</th>';
h+='<th onclick="sortTable(11)">No Supp'+getSortIndicator(11)+'</th>';
h+='<th onclick="sortTable(12)">OOS'+getSortIndicator(12)+'</th>';
h+='<th onclick="sortTable(13)">VRP'+getSortIndicator(13)+'</th>';
h+='<th onclick="sortTable(14)">Price'+getSortIndicator(14)+'</th>';
h+='<th onclick="sortTable(15)">Others'+getSortIndicator(15)+'</th>';
h+='<th onclick="sortTable(16)">No Supp'+getSortIndicator(16)+'</th>';
h+='<th onclick="sortTable(17)">OOS'+getSortIndicator(17)+'</th>';
h+='<th onclick="sortTable(18)">VRP'+getSortIndicator(18)+'</th>';
h+='<th onclick="sortTable(19)">Price'+getSortIndicator(19)+'</th>';
h+='<th onclick="sortTable(20)">Others'+getSortIndicator(20)+'</th>';
h+='<th onclick="sortTable(21)">Forecast'+getSortIndicator(21)+'</th>';
h+='<th onclick="sortTable(22)">vs Target'+getSortIndicator(22)+'</th>';
h+='<th onclick="sortTable(23)">Target'+getSortIndicator(23)+'</th>';
h+='</tr></thead><tbody>';
plans.forEach(p=>{{
const c=calcProjectedOPS(p.cid,p.current_gms,p.target_gms,p.sshve1_suppression,p.sshve2_suppression);
h+='<tr>';
...
const style=isFallback?'background:#fff3cd;font-style:italic':'';
const title=isFallback?'全CEセラー平均値を使用':'';
