"""
Generate Final SSHVE2 Dashboard with GMS-Weighted No Suppression Rate
Calculates no suppression rate from raw SSHVE2 data filtered by displayed MCIDs
"""
import json
import pandas as pd
...
print("=" * 80)
print("Generating Final SSHVE2 Dashboard with GMS-Weighted No Suppression Rate")
print("=" * 80)
...
# Convert to JSON-serializable format for embedding in HTML
suppression_by_mcid = {}
for mcid, group in df_suppression.groupby('merchant_customer_id'):
    mcid_str = str(mcid)
    # Store category and ops for each ASIN
    suppression_by_mcid[mcid_str] = []
    for _, row in group.iterrows():
        suppression_by_mcid[mcid_str].append({
            'category': row['suppression_category_large'],
            'ops': float(row['t30d_ops']) if pd.notna(row['t30d_ops']) else 0
        })

print(f"✅ Processed suppression data for {len(suppression_by_mcid)} MCIDs")
...
output = f'sshve2_final_dashboard_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html'
...
<div style="font-size:1.3em;font-family:Consolas,monospace;margin-top:5px">0.2807</div>
</div>
<div style="text-align:center">
<div style="font-weight:700;color:#9C27B0;font-size:1.1em">VRP Missing</div>
<div style="font-size:1.3em;font-family:Consolas,monospace;margin-top:5px">0.0963</div>
</div>
<div style="text-align:center">
...
<li>黄色のセル（Sourcing改善幅、削減率）をクリックして数値を入力</li>
<li>一括入力機能で複数のMCIDに同じ値を適用可能（工数削減したい場合のみ使用）</li>
...
// Calculate GMS-weighted no suppression rate from raw data
...
const ops=item.ops||0;
totalOPS+=ops;
if(item.category==='1.no suppression'){{
noSuppOPS+=ops;
}}
}});
}}
...
// Calculate GMS-weighted no suppression rate from raw data (SSHVE2)
...
case 5:valA=a.current_gms;valB=b.current_gms;break;
case 6:valA=a.gap;valB=b.gap;break;
case 7:valA=a.target_gms;valB=b.target_gms;break;
case 8:valA=a.event_participation.past_participation?.sshve1_gms||0;valB=b.event_participation.past_participation?.sshve1_gms||0;break;
case 9:valA=a.event_participation.past_participation?.t365_gms||0;valB=b.event_participation.past_participation?.t365_gms||0;break;
case 10:valA=a.sshve1_suppression['No suppression'];valB=b.sshve1_suppression['No suppression'];break;
case 11:valA=a.sshve1_suppression['OOS'];valB=b.sshve1_suppression['OOS'];break;
case 12:valA=a.sshve1_suppression['VRP Missing'];valB=b.sshve1_suppression['VRP Missing'];break;
case 13:valA=a.sshve1_suppression['Price Error'];valB=b.sshve1_suppression['Price Error'];break;
case 14:valA=a.sshve1_suppression['Others'];valB=b.sshve1_suppression['Others'];break;
case 15:valA=a.sshve2_suppression['No suppression'];valB=b.sshve2_suppression['No suppression'];break;
case 16:valA=a.sshve2_suppression['OOS'];valB=b.sshve2_suppression['OOS'];break;
case 17:valA=a.sshve2_suppression['VRP Missing'];valB=b.sshve2_suppression['VRP Missing'];break;
case 18:valA=a.sshve2_suppression['Price Error'];valB=b.sshve2_suppression['Price Error'];break;
case 19:valA=a.sshve2_suppression['Others'];valB=b.sshve2_suppression['Others'];break;
case 20:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.forecastOPS;break;}}
case 21:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.forecastOPS-cA.targetOPS;break;}}
case 22:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.targetOPS;break;}}
case 23:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.srcImp;break;}}
case 24:valA=userInputs[a.cid]?.priceErrorReduction||0;valB=userInputs[b.cid]?.priceErrorReduction||0;break;
case 25:valA=userInputs[a.cid]?.otherSuppReduction||0;valB=userInputs[b.cid]?.otherSuppReduction||0;break;
case 26:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.suppImp;break;}}
case 27:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.projOPS;break;}}
case 28:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.incOPS;break;}}
case 29:{{const cA=calcProjectedOPS(a.cid,a.current_gms,a.target_gms,a.sshve1_suppression,a.sshve2_suppression);valA=cA.vsTarget;break;}}
default:return 0;
}}
if(colIndex>=20&&colIndex<=29){{
const cB=calcProjectedOPS(b.cid,b.current_gms,b.target_gms,b.sshve1_suppression,b.sshve2_suppression);
switch(colIndex){{
case 20:valB=cB.forecastOPS;break;
case 21:valB=cB.forecastOPS-cB.targetOPS;break;
case 22:valB=cB.targetOPS;break;
case 23:valB=cB.srcImp;break;
case 26:valB=cB.suppImp;break;
case 27:valB=cB.projOPS;break;
case 28:valB=cB.incOPS;break;
case 29:valB=cB.vsTarget;break;
}}
}}
...
h+='<th colspan="3">Sourced GMS</th><th colspan="2">今回不参加だが<br>過去参加ASINの<br>T30 GMS</th>';
h+='<th colspan="5">SSHVE1 Suppression</th><th colspan="5">SSHVE2 Suppression</th>';
h+='<th colspan="3">Promotion OPS</th>';
h+='<th rowspan="2" onclick="sortTable(23)">Sourcing<br>改善幅'+getSortIndicator(23)+'</th>';
h+='<th rowspan="2" onclick="sortTable(24)">Price Error<br>削減率%'+getSortIndicator(24)+'</th>';
h+='<th rowspan="2" onclick="sortTable(25)">その他Supp<br>削減率%'+getSortIndicator(25)+'</th>';
h+='<th rowspan="2" onclick="sortTable(26)">Suppression<br>改善幅'+getSortIndicator(26)+'</th>';
h+='<th rowspan="2" onclick="sortTable(27)">Projected<br>OPS'+getSortIndicator(27)+'</th>';
h+='<th rowspan="2" onclick="sortTable(28)">Incremental<br>OPS'+getSortIndicator(28)+'</th>';
h+='<th rowspan="2" onclick="sortTable(29)">vs Target<br>(Projected OPS-Target OPS)'+getSortIndicator(29)+'</th></tr>';
h+='<tr>';
h+='<th onclick="sortTable(5)">Act'+getSortIndicator(5)+'</th>';
h+='<th onclick="sortTable(6)">vs Target'+getSortIndicator(6)+'</th>';
h+='<th onclick="sortTable(7)">Target'+getSortIndicator(7)+'</th>';
h+='<th onclick="sortTable(8)">SSHVE1'+getSortIndicator(8)+'</th>';
h+='<th onclick="sortTable(9)">T365'+getSortIndicator(9)+'</th>';
h+='<th onclick="sortTable(10)">No Supp'+getSortIndicator(10)+'</th>';
h+='<th onclick="sortTable(11)">OOS'+getSortIndicator(11)+'</th>';
h+='<th onclick="sortTable(12)">VRP'+getSortIndicator(12)+'</th>';
h+='<th onclick="sortTable(13)">Price'+getSortIndicator(13)+'</th>';
h+='<th onclick="sortTable(14)">Others'+getSortIndicator(14)+'</th>';
h+='<th onclick="sortTable(15)">No Supp'+getSortIndicator(15)+'</th>';
h+='<th onclick="sortTable(16)">OOS'+getSortIndicator(16)+'</th>';
h+='<th onclick="sortTable(17)">VRP'+getSortIndicator(17)+'</th>';
h+='<th onclick="sortTable(18)">Price'+getSortIndicator(18)+'</th>';
h+='<th onclick="sortTable(19)">Others'+getSortIndicator(19)+'</th>';
h+='<th onclick="sortTable(20)">Forecast'+getSortIndicator(20)+'</th>';
h+='<th onclick="sortTable(21)">vs Target'+getSortIndicator(21)+'</th>';
h+='<th onclick="sortTable(22)">Target'+getSortIndicator(22)+'</th>';
h+='</tr></thead><tbody>';
plans.forEach(p=>{{
const c=calcProjectedOPS(p.cid,p.current_gms,p.target_gms,p.sshve1_suppression,p.sshve2_suppression);
h+='<tr>';
...
window.currentPlans.forEach(p=>{{
const c=calcProjectedOPS(p.cid,p.current_gms,p.target_gms,p.sshve1_suppression,p.sshve2_suppression);
const row=[p.cid,p.merchant_name||'-',p.alias||'-',p.mgr||'-',p.team||'-',p.current_gms,p.gap,p.target_gms,
