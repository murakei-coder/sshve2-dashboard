"""
Create a standalone HTML dashboard for SSHVE2 Bridge Plan
All data is embedded directly in the HTML file - no external dependencies
"""
...
print("=" * 70)
print("Creating Standalone SSHVE2 Dashboard...")
print("=" * 70)
...
<head>
    ...
    <title>SSHVE2 Bridge Plan Dashboard - Standalone</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="info-box">
                <strong>💡 使い方:</strong>
                <p>Team/Mgr/Alias/MCIDのいずれかを選択してから「🔍 検索」ボタンをクリックしてください。<br>
                黄色のセルをクリックして数値を入力すると、Projected OPSが自動計算されます。</p>
            </div>
...
{alias_options}
    ...
    <script>
        ...
        function calculateProjectedOPS(cid, currentGMS, targetGMS, sshve2Supp) {{
            ...
            // Current Price Error percentage
            const currentPriceErrorPct = sshve2Supp['Price Error'] / 100;
            
            // Suppression improvement from Price Error reduction
        ...
        function makeEditable(cell, cid, field) {{
            ...
            input.onblur = function() {{ 
                updateValue(cid, field, this.value); 
            }};
        ...
        function renderTable(plans) {{
            ...
            html += '<th colspan="5">SSHVE2 Suppression Rate</th>';
            ...
            plans.forEach(plan => {{
                ...
                html += '<tr>';
                html += `<td>${{plan.cid}}</td>`;
                html += `<td>${{plan.alias}}</td>`;
                html += `<td>${{plan.mgr}}</td>`;
                html += `<td>${{plan.team}}</td>`;
                ...
                html += `<td class="number editable-cell" onclick="makeEditable(this, '${{plan.cid}}', 'sourcing')" title="クリックして編集">${{formatNumber(calc.sourcingImprovement)}}</td>`;
                html += `<td class="number editable-cell" onclick="makeEditable(this, '${{plan.cid}}', 'priceErrorReduction')" title="クリックして編集">${{formatPercent(userInputs[plan.cid]?.priceErrorReduction || 0)}}</td>`;
                html += `<td class="number calculated-cell ${{calc.achievementRate >= 100 ? 'positive' : 'negative'}}" title="達成率: ${{calc.achievementRate.toFixed(1)}}%">${{formatNumber(calc.projectedOPS)}}</td>`;
                html += '</tr>';
            }});
        ...
        function resetFilter() {{
            ...
            document.getElementById('resultsSection').innerHTML = '<div class="empty-state"><h3>📋 フィルターを選択してください</h3><p>Team、Mgr、Alias、またはMCIDを選択して検索ボタンをクリックしてください</p></div>';
            document.getElementById('statsSection').innerHTML = '';
        }}
        
        function exportToCSV() {{
            if (!window.currentPlans || window.currentPlans.length === 0) {{
                alert('⚠️ データがありません。まず検索を実行してください。');
                return;
            }}
            
            const headers = ['MCID', 'Alias', 'Mgr', 'Team', 'Current GMS', 'Target GMS', 'Gap', 
                           'SSHVE1 ASINs', 'T365 ASINs', 'No Supp %', 'OOS %', 'VRP Miss %', 
                           'Price Err %', 'Others %', 'Forecast OPS', 'vs Target OPS', 
                           'Sourcing Improvement', 'Price Error Reduction %', 'Projected OPS'];
...
output_file = f'sshve2_dashboard_standalone_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html'
with open(output_file, 'w', encoding='utf-8') as f:
    f.write(html_content)

print(f"\n✅ Created standalone HTML dashboard: {output_file}")
print(f"📊 File size: {len(html_content) / 1024 / 1024:.2f} MB")
print("\n" + "=" * 70)
print("✨ Done! Open the HTML file in any browser - no Python needed!")
print("=" * 70)
