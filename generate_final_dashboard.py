"""
Generate Final SSHVE2 Dashboard with Corrected Data and Calculations
"""
import json
from datetime import datetime

print("=" * 80)
print("Generating Final SSHVE2 Dashboard")
print("=" * 80)

# Load corrected data
json_file = 'sshve2_data_new_suppression_20260318_215457.json'
print(f"\nLoading: {json_file}")
...
<title>SSHVE2 Final Dashboard</title>
<style>
...
<div class="container">
<div class="header">
<h1>🎯 SSHVE2 Final Dashboard</h1>
<p>📊 {len(data['cids'])} MCIDs | 🔄 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
</div>
<div class="content">
<div class="info-box">
<strong>✅ 修正済み:</strong>
<p>• Merchant Name: 正しいファイルから取得<br>
• Sourced GMS: SSHVE2_SourcedFlag=Yのみ<br>
• 過去参加ASIN: SSHVE2_SourcedFlag=N & 対象=Y<br>
• 計算ロジック: Projected OPS = Σ(Improved GMS × Improved SSHVE2% × 係数)</p>
</div>
...
// Calculate weighted average no suppression rate
let totalOPS=0;
let noSuppOPS=0;
plans.forEach(p=>{{
const ops=p.current_gms*p.sshve2_suppression['No suppression']/100*COEF['No suppression'];
totalOPS+=ops;
noSuppOPS+=ops;
}});
const noSuppRate=plans.length>0?(plans.reduce((s,p)=>s+p.sshve2_suppression['No suppression'],0)/plans.length).toFixed(1):0;
document.getElementById('statsSection').innerHTML=`
<div class="stats">
<div class="stat-card"><h4>📊 Sourced GMS</h4><div class="value">${{fmt(totCur)}}</div></div>
<div class="stat-card"><h4>🎯 Target GMS</h4><div class="value">${{fmt(totTgt)}}</div></div>
<div class="stat-card"><h4>📈 Gap</h4><div class="value ${{gap>=0?'positive':'negative'}}">${{fmt(gap)}}</div></div>
<div class="stat-card"><h4>✅ No Suppression Rate</h4><div class="value">${{noSuppRate}}%</div></div>
</div>`;
}}
...
print("\n" + "=" * 80)
print("✨ 完成! 全ての修正が適用されました:")
print("  ✅ Merchant Name: 正しいファイルから取得")
print("  ✅ Sourced GMS: SSHVE2_SourcedFlag=Yのみ")
print("  ✅ 過去参加ASIN: SSHVE2_SourcedFlag=N & 対象=Y")
print("  ✅ 計算ロジック: 正しい式を使用")
