...
class DataLoader:
    """Loads raw data files into DataFrame with support for large files."""
    ...
    def load(
        ...
        Supports chunk reading for large files.
        
        Args:
            file_path: Path to the input file
            encoding: File encoding (default: utf-8)
            chunksize: If specified, read file in chunks and concatenate.
                      Useful for large files to avoid memory issues.
                      Recommended: 100000 for files > 1GB
        ...
        if chunksize is not None:
            return self._load_with_chunks(file_path, encoding, chunksize)
    ...
    def _load_with_chunks(
        ...
        try:
            ...
            logger.info(f"Loaded {len(df)} records from {file_path} (chunked)")
            return df
            ...
            try:
                ...
                df = pd.concat(chunks, ignore_index=True)
                logger.info(f"Loaded {len(df)} records from {file_path} (chunked, cp932)")
                return df
            except Exception:
                pass
        except FileNotFoundError:
            ...
            logger.warning(f"Tab-separated chunked parse failed: {e}")
        ...
        try:
            ...
            df = pd.concat(chunks, ignore_index=True)
            logger.info(f"Loaded {len(df)} records from {file_path} (CSV, chunked)")
            return df
        except Exception as e:
            raise ParseError(f"データのパースに失敗しました: {e}")
