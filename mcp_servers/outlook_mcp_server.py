...
def validate_days_parameter(days: int) -> Optional[str]:
    ...
    if days > MAX_DAYS:
        return f"Error: days cannot exceed {MAX_DAYS}"
    return None

def validate_folder_parameters(max_results: int, max_depth: int) -> Optional[str]:
    """Validate folder listing parameters"""
    if not isinstance(max_results, int) or max_results < 1:
        return "Error: max_results must be a positive integer"
    if max_results > MAX_FOLDER_RESULTS:
        return f"Error: max_results cannot exceed {MAX_FOLDER_RESULTS}"
    if not isinstance(max_depth, int):
        return "Error: max_depth must be an integer"
    if max_depth < -1:
        return "Error: max_depth must be -1 (unlimited) or a non-negative integer"
    if max_depth > MAX_FOLDER_DEPTH:
        return f"Error: max_depth cannot exceed {MAX_FOLDER_DEPTH}"
    return None

def validate_email_parameters(recipient_email: str, subject: str, body: str) -> Optional[str]:
    """Validate email composition parameters"""
    if not recipient_email or not recipient_email.strip():
        return "Error: recipient_email cannot be empty"
    if "@" not in recipient_email:
        return "Error: recipient_email must be a valid email address"
...
def should_include_folder(folder_name: str, include_system: bool, system_folders: set) -> bool:
    """Check if folder should be included based on system folder filtering"""
    if include_system:
        return True
    return folder_name.lower() not in system_folders

def format_folder_line(folder_name: str, depth: int) -> str:
    """Format folder display line with proper indentation"""
    prefix = "  " * depth
    return f"{prefix}- {folder_name}\n"

def get_subfolders(folder) -> list:
    """Safely get list of subfolders"""
    try:
        return list(folder.Folders)
    except:
        return []

def list_folders_iterative(root_folders, max_results: int, max_depth: int, include_system: bool, system_folders: set) -> str:
    """List folders using iterative approach with stack"""
    result = ""
    count = 0
    
    # Stack structure: (folder, depth)
    stack = [(folder, 0) for folder in reversed(root_folders)]
    ...
    while stack and count < max_results:
        ...
        if not should_include_folder(folder.Name, include_system, system_folders):
            continue
            
        # Add to result
        result += format_folder_line(folder.Name, depth)
        count += 1
        
        # Add subfolders to stack if not at max depth
...
# MCP Tools
@mcp.tool()
def list_folders(max_results: int, max_depth: int, include_system: bool) -> str:
    """
    List all available mail folders in Outlook with advanced filtering
    
    Args:
        max_results: Maximum number of folders to return (required, 1-1000)
        max_depth: Maximum depth for recursive listing, -1 for unlimited (required, -1 to 20)
        include_system: Include system folders like Deleted Items, Junk Email (required)
    
    Returns:
        A list of available mail folders
    """
    error = validate_folder_parameters(max_results, max_depth)
    if error:
        return error
    ...
    try:
        ...
        system_folders = {"deleted items", "junk email", "sync issues", "conflicts", "local failures", "server failures"}
        
        result = f"Available mail folders (max: {max_results}, depth: {max_depth if max_depth != -1 else 'unlimited'}):\n\n"
        folder_count = 0
        
        # List all root folders and their subfolders recursively
        for folder in namespace.Folders:
            if folder_count >= max_results:
                result += f"\n... (showing first {max_results} folders)\n"
                break
                
            folder_name_lower = folder.Name.lower()
            if not include_system and folder_name_lower in system_folders:
                continue
            ...
            if max_depth != 0:
                sub_result = list_folders_iterative(
                    [folder], max_results - folder_count, max_depth, include_system, system_folders
                )
                result += sub_result
                folder_count += sub_result.count('\n')
...
@mcp.tool()
def list_recent_emails(days: int, folder_name: Optional[str]) -> str:
    """
    List email titles from the specified number of days
    
    Args:
        days: Number of days to look back for emails (required, 1-30)
        folder_name: Name of the folder to check (required, None for Inbox)
        
    Returns:
        Numbered list of email titles with sender information
    """
    error = validate_days_parameter(days)
    if error:
        return error
...
@mcp.tool()
def search_emails(search_term: str, days: int, folder_name: Optional[str]) -> str:
    """
    Search emails by contact name or keyword within a time period
    
    Args:
        search_term: Name or keyword to search for (required)
        days: Number of days to look back (required, 1-30)
        folder_name: Name of the folder to search (required, None for Inbox)
...
def reply_to_email_by_number(email_number: int, reply_text: str) -> str:
    ...
    try:
        ...
        return f"Reply saved to drafts for: {email.SenderName} <{email.SenderEmailAddress}>"
    
    except Exception as e:
        return f"Error replying to email: {str(e)}"

@mcp.tool()
def compose_email(recipient_email: str, subject: str, body: str, cc_email: Optional[str]) -> str:
    """
    Compose and save a new email as draft
    
    Args:
        recipient_email: Email address of the recipient (required)
        subject: Subject line of the email (required)
        body: Main content of the email (required)
        cc_email: Email address for CC (required, None if no CC)
        
    Returns:
        Status message indicating success or failure
    """
    error = validate_email_parameters(recipient_email, subject, body)
    if error:
        return error
    ...
    try:
        ...
        return f"Email saved to drafts for: {recipient_email}"
